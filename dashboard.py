...
# -*- coding: utf-8 -*-
"""
Cloudflare IP Monitor - Dashboard
Pure Python web interface for monitoring Cloudflare IPs
...
from monitor import monitor, start_monitoring, stop_monitoring, get_monitor_status
from scanner import run_initial_scan

logger = logging.getLogger(__name__)


# HTML Template stored as constant
...
<body>
    ...
    <script>
        ...
        function renderIPTable(ips) {
            ...
            if (ips.length === 0) {
                tbody.innerHTML = '<tr><td colspan="9" class="empty-state"><div class="empty-state-icon">📭</div><div>No IPs found. Run an initial scan.</div></td></tr>';
                return;
            }
            tbody.innerHTML = ips.map(ip => {
                ...
                return '<tr>' +
                    ...
                    '<td><button class="btn btn-sm btn-danger" onclick="deactivateIP(\\''+ip.ip_address+'\\')">✕</button></td>' +
                    '</tr>';
            }).join('');
        }
        ...
        async function loadHourlyChart() {
            ...
            const maxSpeed = Math.max(...data.stats.map(s => s.avg_speed || 0));
        ...
        async function showIPDetails(ip) {
            ...
            if (history && history.history && history.history.length > 0) {
                ...
                html += history.history.slice(0, 20).map(h => '<tr><td style="font-size: 0.75rem;">'+new Date(h.test_time).toLocaleString()+'</td><td>'+(h.download_speed_mbps || 0).toFixed(2)+'</td><td>'+(h.latency_ms || 0).toFixed(0)+'</td><td>'+((h.loss_rate || 0) * 100).toFixed(1)+'%</td></tr>').join('');
        ...
        async function startInitialScan() {
            ...
            if (result) { showToast('Initial scan started! This may take several minutes.', 'success'); closeScanModal(); }
        }
        ...
        setInterval(() => { loadIPs(); loadHourlyChart(); }, 120000);
        ...
        document.addEventListener('keydown', (e) => { if (e.key === 'Escape') document.querySelectorAll('.modal-overlay').forEach(m => m.classList.remove('active')); if (e.key === 'r' && document.activeElement.tagName !== 'INPUT') refreshData(); });
    </script>
</body>
</html>'''
...
class DashboardHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        routes = {
            ...
            '/api/ips': self._handle_api_ips,
            '/api/ip': self._handle_api_ip_detail,
            '/api/history': self._handle_api_history,
            '/api/hourly': self._handle_api_hourly,
            '/api/monitor/status': self._handle_api_monitor_status,
        }
    ...
    def do_POST(self):
        ...
        routes = {
            '/api/scan/initial': self._handle_api_initial_scan,
            '/api/scan/test': self._handle_api_test_now,
            '/api/monitor/start': self._handle_api_monitor_start,
            '/api/monitor/stop': self._handle_api_monitor_stop,
            '/api/monitor/interval': self._handle_api_set_interval,
            '/api/ip/deactivate': self._handle_api_deactivate_ip,
        }
    ...
    def _handle_api_stats(self, query: Dict):
        ...
        stats['monitor'] = get_monitor_status()
        self._send_json(stats)
    
    def _handle_api_ips(self, query: Dict):
        order_by = query.get('sort', 'avg_download_speed')
        order_dir = 'DESC' if query.get('dir', 'desc').upper() == 'DESC' else 'ASC'
        search = query.get('search', None)
        active_only = query.get('active', 'true').lower() == 'true'
        ips = db.get_all_ips_with_stats(order_by=order_by, order_dir=order_dir, search=search, active_only=active_only)
        self._send_json({"ips": ips, "total": len(ips)})
    
    def _handle_api_ip_detail(self, query: Dict):
        ip = query.get('ip')
        if not ip:
    ...
    def _handle_api_history(self, query: Dict):
        ip = query.get('ip')
        hours = int(query.get('hours', 24))
        if not ip:
            self._send_json({"error": "IP required"}, 400)
            return
        history = db.get_ip_history(ip, hours)
        self._send_json({"ip": ip, "history": history})
    
    def _handle_api_hourly(self, query: Dict):
        hours = int(query.get('hours', 24))
        stats = db.get_hourly_stats(hours)
        self._send_json({"hours": hours, "stats": stats})
    
    def _handle_api_monitor_status(self, query: Dict):
        self._send_json(get_monitor_status())
    
    def _handle_api_initial_scan(self, data: Dict):
        def run_scan():
        ...
        thread.start()
        self._send_json({"status": "started", "message": "Initial scan started in background"})
    
    def _handle_api_test_now(self, data: Dict):
        def run_test():
            results = monitor.trigger_immediate_test()
            logger.info(f"Immediate test completed: {len(results)} results")
        
        thread = threading.Thread(target=run_test, daemon=True)
        thread.start()
        self._send_json({"status": "started", "message": "Test started"})
    
    def _handle_api_monitor_start(self, data: Dict):
        interval = data.get('interval', 120)
        start_monitoring(interval, run_immediately=True)
        self._send_json({"status": "started", "interval": interval})
    
    def _handle_api_monitor_stop(self, data: Dict):
        stop_monitoring()
        self._send_json({"status": "stopped"})
    
    def _handle_api_set_interval(self, data: Dict):
        interval = data.get('interval', 120)
        monitor.set_interval(interval)
        self._send_json({"status": "updated", "interval": interval})
    
    def _handle_api_deactivate_ip(self, data: Dict):
        ip = data.get('ip')
        if not ip:
            self._send_json({"error": "IP required"}, 400)
            return
        db.deactivate_ip(ip)
        self._send_json({"status": "deactivated", "ip": ip})
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Cloudflare IP Monitor Dashboard')
    parser.add_argument('--host', default='0.0.0.0', help='Host to bind to')
    parser.add_argument('--port', type=int, default=8080, help='Port to listen on')
    parser.add_argument('--no-monitor', action='store_true', help='Do not auto-start monitor')
    
    args = parser.parse_args()
    
    run_dashboard(host=args.host, port=args.port, auto_start_monitor=not args.no_monitor)
